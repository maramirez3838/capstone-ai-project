...
**Version:** 1.7
**Date:** April 21, 2026
**Derived From:** STR Comply PRD v2.3
...
### 15C.1 Market Ingestion Agent (`lib/agents/market-ingestion-agent.ts`) — Sprint 4

**Intent:** Research a new city's STR rules using Claude Sonnet + `web_search` and generate a complete market record (rules, sources, summary) for maintainer review before DB insert.

**Key interfaces:**
- `MarketIngestionInput`: `{ cityName, stateCode, countyName? }`
- `MarketIngestionResult`: `{ cityName, stateCode, countyName, slug, strStatus, permitRequired, ownerOccupancyRequired, summary, rules[], sources[], confidenceScore, reviewNotes[] }`
- `IngestionCandidateRule`: mirrors `MarketRule` fields plus `applicableTo` and `jurisdictionLevel`
- `IngestionCandidateSource`: mirrors `MarketSource` fields

**Implementation notes for Sprint 4:**
- Cost model: Haiku pre-screen (assess data availability) → Sonnet full research
- All ingested markets insert with `supportStatus: 'pending_review'` — no auto-publish
- Sources must pass the section-anchored URL requirement (see 15A.9) before insert
...
15. Sprint 3 — Property Requirements Agent ← **next**
    - Implement `lib/agents/property-requirements-agent.ts` (see section 15D)
    - Create `app/api/property/requirements/route.ts` (see section 14.9)
16. Sprint 4 — Market Ingestion Agent (see section 15C.1)
17. Tests and README polish
