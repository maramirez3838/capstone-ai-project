...
Product Requirements Document | v2.5 | Desktop-First Capstone Revision

| Author | Product Management Team |
| :---- | :---- |
| **Status** | Revised — Property Watchlist + Change-Alert Infrastructure |
| **Date** | April 2026 |
| **Phase** | Capstone Feasibility \+ MVP Validation |
| **Primary Persona** | STR Investor (Marcus) |

## Changelog
- v2.5 (2026-04-25): **Property watchlist + change-alert infrastructure (BE only).** Watchlist is now polymorphic: users can save markets *and* properties as independent 25-item lists, surfaced via a Markets | Properties tab switcher on `/watchlist`. Schema: `WatchlistItem.marketId` made nullable; new nullable `propertyId` + property FK with cascade; `(userId, propertyId)` unique. New `WatchlistButton` accepts a discriminated `kind: 'market' | 'property'` prop; legacy market call sites updated. **Change-alert infrastructure layer added** (BE only this sprint, gated off by `NOTIFICATIONS_ENABLED=false`): `MarketChangeEvent` log persists every detected market refresh diff with computed severity (high / medium / low). Per-user `NotificationPreference` (lazy-init defaults: alerts on, min severity low, email channel, immediate cadence). `Notification` outbox + audit table with industry-standard idempotency (unique on `(userId, changeEventId, watchKind, watchTargetId)`), retry with exponential backoff (1m / 5m / 30m, 3 attempts), and dead-letter `failed` state. RFC 8058 one-click unsubscribe via HMAC token (90d TTL, separate `UNSUBSCRIBE_SECRET`). Vercel cron `/api/cron/notifications` drains the outbox every 15 min. `/api/admin/ingest-market` now writes a `MarketChangeEvent` and fans out to subscribers inline after every refresh. Test count: 105 → 146. Deferred to future sprints: notification preference UI, in-app feed, daily/weekly digest delivery, SMS/push channels, N4 rule-diff UI surface.
- v2.4 (2026-04-25): UI sprint — property and market searches are now first-class, switchable destinations. Address-resolved searches land on a new `/property` page that renders per-address compliance requirements (previously dark — backend was shipped in Sprint 3 but had no UI). A two-tab `PropertyMarketSwitcher` lets users flip between "this property" and "{market} overall" via real navigation, preserving URL, refresh, and browser back. Market-name searches keep the existing flow. Backlog items deferred to follow-ups: property pinning in watchlist, `needs_review` rule diff visibility, source-status copy clarity audit.
- v2.3 (2026-04): Source Discovery Agent + Approval Flow added; RICE table updated; refresh model and constraints updated.
