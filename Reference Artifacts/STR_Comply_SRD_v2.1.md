...
**Version:** 2.1
**Date:** April 25, 2026
**Derived From:** STR Comply PRD v2.5
...
## Changelog
- v2.1 (2026-04-25): **Property watchlist (polymorphic) + change-alert infrastructure layer.**
  **Schema (migration `watchlist_property_polymorphism_and_alerts`):** `WatchlistItem.marketId` made nullable, new nullable `WatchlistItem.propertyId` with FK to `Property` and cascade delete, new `@@unique([userId, propertyId])`. XOR invariant (exactly one of marketId/propertyId set) enforced in the POST handler — Postgres NULLs are distinct in unique indexes so the two unique constraints don't collide. Three new tables: `MarketChangeEvent` (id, marketId FK cascade, rulesVersionFrom/To, changeSummary JSONB, severity, detectedAt; index on `(marketId, detectedAt)`); `NotificationPreference` (one per user via `@@unique`, lazy-init defaults: `alertsEnabled=true`, `emailEnabled=true`, `minSeverity='low'`, `digestFrequency='immediate'`); `Notification` (outbox + audit, `@@unique([userId, changeEventId, watchKind, watchTargetId])` for fan-out idempotency, indexes on `(status, createdAt)` and `(userId, sentAt)`; status: `queued | sent | failed | suppressed`; channel: `email`).
  **Routes:** `POST /api/watchlist` body is now a Zod union (`{ marketSlug } | { propertyAddress }`); `GET /api/watchlist` returns `{ markets: WatchlistMarketEntry[], properties: WatchlistPropertyEntry[] }` (breaking change to the internal contract; only the hook + listing page consume it). New `DELETE /api/watchlist/property/[propertyId]` mirrors the market DELETE (cuid in URL because addresses are too long/messy). New `GET /api/notifications/unsubscribe?token=…` (RFC 8058 one-click). New `GET /api/cron/notifications` (Vercel cron entry — drains the outbox).
  **Modules under `lib/notifications/`:** `change-event.ts` (RuleDiff type, `classifyChangeSeverity`, `writeChangeEvent` with empty-diff short-circuit); `fanout.ts` (`fanOutChangeEvent` joins direct market watchers + indirect property watchers, applies per-user prefs with severity threshold, upserts on the unique key for idempotency, suppression policy: `alertsEnabled=false` → no row, `severity < minSeverity` → row with `status='suppressed'`); `outbox.ts` (`processOutbox` with batch + backoff `[0, 1m, 5m, 30m]` + maxAttempts 3 + Resend dispatch + context-not-found handling); `email-templates.ts` (market + property variants, both include the project disclaimer + one-click unsubscribe footer).
  **Helpers:** `lib/unsubscribe-token.ts` mirrors `lib/approval-token.ts` but with 90d TTL and a separate `UNSUBSCRIBE_SECRET` so a leaked unsubscribe token cannot forge auth tokens.
  **Wiring:** `/api/admin/ingest-market` calls `writeChangeEvent` then `fanOutChangeEvent` inline after the existing `rulesVersion` bump. Outbox processor is gated by `NOTIFICATIONS_ENABLED=true` so unfinished plumbing never sends real emails by accident.
  **Cron:** `vercel.json` entry `/api/cron/notifications` every 15 minutes. Daily/weekly digest cadences are deferred (column accepts the values; processor only honors `immediate`).
  **Env vars added (`.env.example`):** `NOTIFICATIONS_ENABLED`, `UNSUBSCRIBE_SECRET`, `NEXT_PUBLIC_APP_URL`.
  **Telemetry events added** (CLAUDE.md approved list): `property_saved`, `property_removed`, `change_event_written`, `notification_queued`, `notification_sent`, `notification_unsubscribed`.
  **Test count: 105 → 146 across 14 files.** New: `__tests__/api/watchlist-property.test.ts`, `__tests__/notifications/change-event.test.ts`, `__tests__/notifications/fanout.test.ts`, `__tests__/notifications/outbox.test.ts`, `__tests__/api/unsubscribe.test.ts`.
- v2.0 (2026-04-25): **Property requirements caching** — agent output is now persistent. Added 5 columns to `Property` (`requirementsJson`, `requirementsConfidenceNote`, `requirementsReviewFlags`, `requirementsGeneratedAt`, `requirementsRulesVersion`) and `Market.rulesVersion` as the invalidation key. New helper `lib/market-rules-version.ts` computes a deterministic SHA-256 hash of the market's rules + status fields (sorted, order-independent) — 7 unit tests. `Market.rulesVersion` is bumped at two write sites: the seed script (after rules insert) and the `/api/admin/ingest-market` route (after the Market Refresh Agent replaces rules). The `/api/property/requirements` route now: reads `Market.rulesVersion`, compares to `Property.requirementsRulesVersion`, returns the cached row on match (with `cached: true`, `cachedAt`, `rulesVersion`) — otherwise runs the agent and persists. First-call latency unchanged (~30s). Cache hits measured at <300ms (134× faster). Aligns property requirements with the PRD principle that monitor agents are the only path that updates existing data. Migration: `add_property_requirements_cache`. Test count: 90 → 105.
